...
try:
    gemini_client = genai.Client(
        vertexai=True,
        project="phrasal-alpha-499105-s2",
...
async def completar_reto_command(interaction: discord.Interaction, reto_id: str, pr_url: str):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            "UPDATE participantes_retos SET estado = 'completado', fecha_fin = ?, pr_url = ? WHERE user_id = ? AND reto_id = ?",
            (now_iso, pr_url, interaction.user.id, reto_id)
        )
        await db.commit()
    await interaction.response.send_message(f"🎉 ¡Felicidades! Has completado el reto `{reto_id}`.\n🔗 PR: <{pr_url}>")
